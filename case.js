...
module.exports = haruka = async (haruka, m, msg, chatUpdate, store) => {
	try {
		...
		switch (command) {		
			...
			break;
case 'loli': {
const url = "https://api.lionzy.xyz/api/loli"
await reactionMessage("⏱️")
const response = await axios.get(url)
let buff = Buffer.from(response.data)
await haruka.sendMessage(m.chat,{
image : buff,
caption : `Lolicon jir😹\nNih kak ${pushname},${command}-nya`,
buttons : [{
...
case 'randomba':case 'rba':{
    const urlo = "https://api.lionzy.xyz/api/randomba";
    haruka.sendMessage(m.chat,{
        image : {url: urlo},
        caption : `Nih kak ${pushname}`,
        buttons: [{
            buttonId: `${prefix}${command}`,
            buttonText : {
...
break
case 'sertifikat':{
if (!text) return newReply("Textnya mana??");
...
case 'kalender': {
 ...
 // Ngadapetkeun taun jeung bulan ayeuna atawa tina argumen
 ...
 ctx.fillText(`KALENDER ${year}`, 50, 50);
 ...
 ctx.fillText(day, dayX, 150);
 dayX += 120;
 });

 // Garis horizontal handap header poé
 ctx.strokeStyle = '#333';
 ctx.lineWidth = 2;
 ctx.beginPath();
 ctx.moveTo(50, 160);
 ...
 // Nampilin tabel tanggal jeung format kalender
 let dateX = 50;
 let dateY = 200;
 ctx.font = '20px Arial';
 ...
 if (date) {
 // Nambihan lingkaran keur tanggal ayeuna
 if (date === currentDate && startDate.month() === moment().month() && startDate.year() === moment().year()) {
 ctx.beginPath();
 ctx.arc(dateX + 10, dateY - 15, 20, 0, 2 * Math.PI, false);
 ctx.fillStyle = '#FFD700';
 ctx.fill();
 ctx.fillStyle = '#000000'; // Ubah teks tanggal ayeuna jadi hideung
 } else if (dayNames[index % 7] === 'Minggu') {
 ctx.fillStyle = '#FF0000'; // Warna beureum keur poé Minggu
 } else {
 ...
 await haruka.sendMessage(m.chat, { image: buffer, caption: "Kalender untuk bulan ini" }, { quoted: m });
}
break
...
case 'zeya':{
				...
				if (!m.isGroup) {
					try {
					    let idsesi = m.sender.split('@')[0]
						const data = await fetchJson(`https://api.lionzy.xyz/api/zeyaai?message=${encodeURIComponent(text)}&sessionId=${idsesi}`);
						haruka.sendMessage(m.chat, {
							text: data.text, 
							ai: true
						}, {
					...
					try {
						...
						const data = await fetchJson(`https://api.lionzy.xyz/api/zeyaai?message=${encodeURIComponent(text)}`);
						haruka.sendMessage(m.chat, {
							text: data.text, 
							ai: false
						}, {
...
if (!json.access_token) return resolve({ status: false, msg: "Can't generate token!" })
...
if (!json.tracks.items || json.tracks.items.length < 1) return resolve({ status: false, msg: 'Music not found!' })
let metadata = {}
...
resolve({ status: true, metadata, audio: { url } })
} catch (e) {
resolve({ status: false, msg: e.message })
}
})
...
haruka.sendMessage(m.chat, { image: { url: kont.metadata.thumbnail }, caption: `Title: ${kont.metadata.title} Artist: ${kont.metadata.artist} Name: ${kont.metadata.name} Preview Url: ${kont.metadata.preview} Popularity: ${kont.metadata.popularity} Url: ${kont.metadata.url}`}, { quoted: null })
await haruka.sendMessage(m.chat, { audio: { url: kont.audio.url }, mimetype: 'audio/mpeg', fileName: `${kont.metadata.title}.mp3` }, { quoted: m });
}
break
...
if (!text) return newReply(`*• Example :* ${prefix + command} *[mediafire url]*`);

const mediafireRegex = /^(https?:\/\/)?(www\.)?mediafire\.com\/.+$/i;
if (!mediafireRegex.test(text)) {
 return newReply(`*• Example :* ${prefix + command} *[mediafire url]*`);
}
...
try {
 ...
 await haruka.sendFile(m.chat, url, filename, cpt, m, false, {
...
case 'delcase':case 'dc':{
if (!isCreator) return newReply(mess.owner);
if (!text) return m.reply("Nama case?")
...
dellCase('case.js',text)
newReply(`Sukses menghapus case ${text}`)
...
${txtVoice}`
 ...
 haruka.sendMessage(m.chat, { audio: { url: ba.result.url }, mimetype: 'audio/mpeg', ptt: true }, { quoted: m })
 } catch (err) {
 console.error(err);
 m.reply('Yahh, error...')
 }
...
Example:
 .fetcher --url https://example.com --headers '{"User-Agent": "MyApp"}' --timeout 5000 --cookie "sessionid=abc123"
 `);
 }
 ...
 if (timeout < 0) {
 return m.reply('Timeout tidak boleh negatif.');
 }

 if (getBufferOnly && onlyHeaders) {
 return m.reply('Hanya satu dari --get-buffer dan --only-headers yang bisa digunakan.');
 }

 if (!axios && !fetch && !cloudscraper && !undici) {
 return m.reply('Tidak ada module yang dipilih.');
 }
 ...
 response = await fetchh(url, { headers: finalHeaders, timeout });
 return await response.text(); // return HTML content
 } else if (cloudscraper) {
 response = await cloudscraperr.get(url, { headers: finalHeaders, timeout });
 return response; // return HTML content
 } else if (undici) {
 response = await undiciFetch(url, { headers: finalHeaders, timeout });
 return await response.text(); // return HTML content
 } else {
 return m.reply('Module tidak bisa digunakan.');
 }
 }
 ...
 m.reply(typeof result === 'string' ? result : JSON.stringify(result, null, 2));

 await fs.writeFileSync("./tes.html", typeof result === 'string' ? result : JSON.stringify(result, null, 2));

 await haruka.sendMessage(
 m.chat,
 {
 document: await fs.readFileSync("./tes.html"),
 fileName: "tes.html",
 mimetype: "text/html",
 },
 { quoted: m }
...
case 'mari':{
				...
				if (!m.isGroup) {
					try {
						...
						const data = await fetchJson(`https://api.lionzy.xyz/api/mariai?message=${encodeURIComponent(text)}`);
						haruka.sendMessage(m.chat, {
							text: data.message, 
							ai: true
						}, {
...
case 'balogo': {
 ...
 if (!kiri || !kanan) return m.reply('Mana textnya? Contoh: .balogo rynn|apalaa')
 await m.reply(' Waitt...')
 const logo = await balogo([kiri, kanan])
 const res = await logo.toBuffer()
 haruka.sendMessage(m.chat, { image: res }, { quoted: m })
 } catch (err) {
 console.error(err)
...
case 'tagsw': {
 if (!isCreator) return m.reply("Siapa???")
 if (!text) return newReply(`Masukkan teks untuk status atau reply gambar/video dengan caption`);
 let media = null;
 let options = {};
 ...
 return haruka.sendMessage("status@broadcast", options, {
 backgroundColor: "#7ACAA7",
 textArgb: 0xffffffff,
 font: 1,
 statusJidList: await (await haruka.groupMetadata(m.chat)).participants.map((a) => a.id),
 additionalNodes: [
 {
 tag: "meta",
 attrs: {},
...
case 'get': {
 // Validasi input URL
 if (!text) return newReply('Awali *URL* dengan http:// atau https://');
 ...
 // Menangani respons berdasarkan tipe konten
 if (/json/i.test(contentType)) {
 const jsonData = JSON.parse(Buffer.from(gt.data, 'binary').toString('utf8'));
 return newReply(JSON.stringify(jsonData, null, 2));
 } else if (/text/i.test(contentType)) {
 const textData = Buffer.from(gt.data, 'binary').toString('utf8');
 return repfut(textData);
 } else if (text.includes('webp')) {
 return haruka.imgToSticker(m.chat, text, m, { packname: "", author: "Hann Universe!!" });
 } else if (/image/i.test(contentType)) {
 return haruka.sendMessage(m.chat, { image: { url: text } }, { quoted: m });
 } else if (/video/i.test(contentType)) {
 return haruka.sendMessage(m.chat, { video: { url: text } }, { quoted: m });
 } else if (/audio/i.test(contentType) || text.includes(".mp3")) {
 return haruka.sendMessage(m.chat, { audio: { url: text } }, { quoted: m });
 } else if (/application\/zip/i.test(contentType) || /application\/x-zip-compressed/i.test(contentType)) {
 return haruka.sendFile(m.chat, text, '', text, m);
 } else if (/application\/pdf/i.test(contentType)) {
 return haruka.sendFile(m.chat, text, '', text, m);
 } else {
 return newReply(`MIME : ${contentType}\n\n${gt.data}`);
 }
 } catch (error) {
 // Menangani kesalahan
 console.error(`Terjadi kesalahan: ${error}`);
 return newReply(`Terjadi kesalahan saat mengakses URL: ${error.message}`);
 }
}
break;
...
const textData = Buffer.from(gt.data, 'binary').toString('utf8');
return newReply(textData);
} else if (text.includes('webp')) {
return haruka.imgToSticker(m.chat, text, m, { packname: "", author: "ZullX!!" })
} else if (/image/i.test(contentType)) { return haruka.sendMessage(m.chat, { image: { url: text }}, { quoted: m });
} else if (/video/i.test(contentType)) { return haruka.sendMessage(m.chat, { video: { url: text }}, { quoted: m });
} else if (/audio/i.test(contentType) || text.includes(".mp3")) {
return haruka.sendMessage(m.chat, { audio: { url: text }}, { quoted: m });
} else if (/application\/zip/i.test(contentType) || /application\/x-zip-compressed/i.test(contentType)) {
return haruka.sendFile(m.chat, text, '', text, m)			
...
case 'deepseek': {
 ...
 await m.reply(' Waitt...')
 const hytam = await blekbox(text)
 m.reply(hytam)
 } catch (err) {
 console.error(err)
...
break
case 'addcase':case 'adc': {
                if (!isCreator) return m.reply(mess.owner)
                if (!text) return newReply('Mana case nya');
                const fs = require('fs');
            ...
            fs.readFile(namaFile, 'utf8', (err, data) => {
                ...
                if (posisiAwalGimage !== -1) {
                    ...
                    fs.writeFile(namaFile, kodeBaruLengkap, 'utf8', (err) => {
                        if (err) {
                            newReply('Terjadi kesalahan saat menulis file:', err);
                        } else {
                            newReply('Case baru berhasil ditambahkan di atas case gimage.');
                        }
                    });
                } else {
                    newReply('Tidak dapat menemukan case gimage dalam file.');
                }
            });
			...
			case 'add': {
				if (!m.isGroup) return newReply(mess.group);
				if (!isCreator) return newReply(mess.admin);
				if (!isBotAdmins) return newReply(mess.botAdmin);
